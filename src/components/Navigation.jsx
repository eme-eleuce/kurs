...
export default function Navigation() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Cerrar cualquier submenu si se hace click fuera
  ...
  const navItems = [
    ...
    { name: "Generador de Reportes", path: "/generador-v2" },
  ...
  return (
    <nav className="bg-white shadow-md border-b border-gray-200 sticky top-0 z-[1200]">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex space-x-4">
            {navItems.map((item, index) => {
              if (item.submenu) {
                const submenuKey = `desktop-${index}`;
                        ...
                        {item.submenu.map((subItem) => {
                          ...
                          return (
                            <Link
                              key={subItem.path}
                              href={subItem.path}
                              onClick={() => setOpenSubmenus(prev => ({ ...prev, [submenuKey]: false }))}
